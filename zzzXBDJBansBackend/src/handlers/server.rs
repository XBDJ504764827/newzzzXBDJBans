...
pub async fn create_server(
    ...
    let verif = payload.verification_enabled.unwrap_or(true);
    let wl = payload.enable_whitelist.unwrap_or(false);


    let result = sqlx::query(
        "INSERT INTO servers (group_id, name, ip, port, rcon_password, verification_enabled, enable_whitelist, min_rating, min_steam_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    )
    ...
    .bind(verif)
    .bind(wl)
    .bind(payload.min_rating.unwrap_or(0.0))
    .bind(payload.min_steam_level.unwrap_or(0))
    .execute(&state.db)
    .await;
