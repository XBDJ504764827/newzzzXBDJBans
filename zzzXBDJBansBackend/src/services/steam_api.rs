...
impl SteamService {
    ...
    pub async fn get_steam_level(&self, steam_id_64: &str) -> Option<i32> {
        // 尝试主 API: api.steampowered.com
        let url_main = format!(
            "https://api.steampowered.com/IPlayerService/GetSteamLevel/v1/?key={}&steamid={}",
            &self.api_key, steam_id_64
        );

        match self.client.get(&url_main).send().await {
            Ok(resp) if resp.status().is_success() => {
                if let Ok(data) = resp.json::<SteamLevelResponse>().await {
                    if let Some(level) = data.response.player_level {
                        return Some(level);
                    }
                }
            }
            _ => {
                tracing::warn!("Main Steam API failed, trying backup (SteamChina)...");
            }
        }

        // 尝试备用 API: api.steamchina.com
        let url_backup = format!(
            "https://api.steamchina.com/IPlayerService/GetSteamLevel/v0001/?key={}&steamid={}&include_appinfo=1",
            &self.api_key, steam_id_64
        );

        match self.client.get(&url_backup).send().await {
            Ok(resp) if resp.status().is_success() => {
                if let Ok(data) = resp.json::<SteamLevelResponse>().await {
                    return data.response.player_level;
                }
            }
            Err(e) => tracing::error!("Backup Steam API Level Error: {}", e),
            Ok(resp) => tracing::error!("Backup Steam API returned status: {}", resp.status()),
        }
        None
    }
